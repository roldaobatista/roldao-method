...
// ============================================================================
{
  ...
  check(
    'T-006.2: nivel block default → [BLOQUEIO]',
    lib.hookPrefix('block', 'x') === '[BLOQUEIO] [x]',
  );
  check('T-006.3: nivel warn → [AVISO]', lib.hookPrefix('warn', 'x') === '[AVISO] [x]');
  check('T-006.4: nivel info → [INFO]', lib.hookPrefix('info', 'x') === '[INFO] [x]');
  check(
    'T-006.5: nivel desconhecido cai pra BLOQUEIO',
    lib.hookPrefix('???', 'x') === '[BLOQUEIO] [x]',
  );
...
// ============================================================================
{
  ...
  check(
    'T-007.2: stderr usa "cabecalho de identificacao"',
    /cabecalho de identificacao/.test(r.stderr),
  ...
  const stderrSemNomeHook = r.stderr
    .split('\n')
    .filter((l) => !l.includes('paths-frontmatter-validator'))
    .join('\n');
  check(
    'T-007.3: texto principal nao usa "frontmatter" (excluindo nome do hook)',
    !/frontmatter/i.test(stderrSemNomeHook),
    `stderr ainda usa "frontmatter": ${stderrSemNomeHook.match(/.*frontmatter.*/i)?.[0] || ''}`,
  );
  check(
    'T-007.4: stderr usa "Efeito" + "Causa" + "Proximo passo"',
    /Efeito:/.test(r.stderr) && /Causa:/.test(r.stderr) && /Proximo passo/.test(r.stderr),
    'falta estrutura EFEITO/CAUSA/PROXIMO',
  );
  check(
    'T-007.5: stderr usa prefixo [BLOQUEIO]',
    /\[BLOQUEIO\]/.test(r.stderr),
...
// ============================================================================
{
  ...
  const r = runHook('paths-frontmatter-validator', input);
  check(
    'T-007.6: cabecalho completo → exit 0',
    r.exit === 0,
...
// ============================================================================
{
  ...
  check(
    'T-007.8: stderr menciona campo faltando + dica de adicao',
...
// ============================================================================
{
  ...
  const src = fs.readFileSync(
    path.join(ROOT, 'templates/.claude/hooks/no-amend-after-push.js'),
    'utf8',
  );
  check(
    'T-008.1: stderr usa "programa Git instalado" em vez de "git no PATH"',
    /programa Git instalado/.test(src),
    'fonte ainda nao usa "programa Git instalado"',
  );
  check(
    'T-008.2: stderr usa "Efeito" + "Causa" + "Proximo passo"',
