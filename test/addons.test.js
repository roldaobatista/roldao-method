...
for (const addon of addons) {
  ...
  for (const field of ['name', 'version', 'description', 'status', 'revisado-em']) {
    check(
      `${addon}: campo '${field}'`,
      !!yaml.top[field],
      `valor: ${yaml.top[field] || '<vazio>'}`,
    );
  }
  check(
    `${addon}: name no YAML bate com diretorio`,
    yaml.top.name === addon,
    `name=${yaml.top.name}`,
  );
  ...
  for (const a of agents) {
    check(
      `${addon}: agente declarado existe (${a})`,
      fs.existsSync(path.join(dir, '.claude/agents', `${a}.md`)),
    );
  }
  for (const h of hooks) {
    ...
    if (fs.existsSync(hookPath)) {
      ...
      try {
        ...
        check(
          `${addon}: hook ${h} syntax (node --check)`,
          false,
          (e.stderr || '').toString().substring(0, 120),
        );
      }
      ...
      check(
        `${addon}: hook ${h} smoke (exit 0 ou 2, nunca crash)`,
        okExit,
        `exit=${exit}, stderr=${(r.stderr || '').toString().substring(0, 100)}`,
      );
    }
  }
  for (const s of skills) {
    check(
      `${addon}: skill declarada existe (${s})`,
      fs.existsSync(path.join(dir, '.claude/skills', s, 'SKILL.md')),
    );
  }
  for (const c of commands) {
    check(
      `${addon}: command declarado existe (${c})`,
      fs.existsSync(path.join(dir, '.claude/commands', `${c}.md`)),
    );
  }
  ...
  check(
    `${addon}: provoca pelo menos 1 item (agent/hook/skill/command)`,
    totalProvoca >= 1,
    `total=${totalProvoca}`,
  );
}
