 ...
 * (+ docs auxiliares) e 35 commands em templates/.claude/, e smoke do statusline.js.
 *
...
check(
  '22 agentes invocáveis em templates/.claude/agents/',
  agentFiles.length === 22,
  `achou ${agentFiles.length}`,
);
...
for (const file of agentFiles) {
  ...
  check(
    `agent ${slug}: description com gatilho concreto (>=80 chars)`,
...
for (const f of auxFiles) {
  ...
  check(
    `agents/${f}: tem frontmatter (owner/revisado-em/status)`,
...
check(
  '35 commands em templates/.claude/commands/',
  cmdFiles.length === 35,
...
for (const file of cmdFiles) {
  ...
  check(
    `command /${slug}: allowed-tools declarado (menor privilégio SEC-004)`,
    !!fm['allowed-tools'],
...
const shardChamaInvestigador =
  /Invoque\s+`?investigador`?|Task\s+subagent_type\s*=\s*investigador/i.test(shardBody);
if (shardChamaInvestigador) {
  check(
    'shard: tem Task em allowed-tools (chama investigador)',
...
if (fs.existsSync(STATUSLINE)) {
  ...
  const tmpTranscript = path.join(
    require('os').tmpdir(),
    `roldao-test-transcript-${Date.now()}.jsonl`,
  );
  ...
  check(
    'statusline: lê transcript_path e calcula % de contexto',
    /📊\s*\d+%/.test(r2.stdout || ''),
...
if (fs.existsSync(OUTPUT_STYLES_DIR)) {
  ...
  check(
    'output-styles: ≥ 3 estilos (pt-br-conciso/dpo-lgpd/fiscal-br)',
    styles.length >= 3,
    `achou ${styles.length}`,
  );
...
if (fs.existsSync(RULES_DIR)) {
  ...
  for (const file of rules) {
    ...
    if (fm) {
      check(
        `rule ${file}: frontmatter coerente (se declarado)`,
