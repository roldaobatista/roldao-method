...
(async () => {
  ...
  const clean = resp.replace(/```[\s\S]*?```/g, '').replace(/`[^`]*`/g, '');
  ...
  for (const m of matches.slice(0, 10)) {
    ...
    if (
      /\(.{0,80}(salv|enviei|atualiz|servidor|cliente|sistema|arquivo|versao|configura).{0,80}\)/i.test(
        ctx,
      )
    )
      continue;
    ...
    if (/block-jargon-pt-br|INV-AGENT-001|hook que bloqueia jarg|tabela de jarg/i.test(ctx))
      continue;
    ...
    if (
      /^#{1,4}\s*(Adicionado|Mudado|Corrigido|Removido|Preservado|Deprecado|Seguranca)\b/i.test(ctx)
    )
      continue;
    if (/CHANGELOG\.md|docs\/releases\/|docs\/checkpoints\/|docs\/postmortems\//i.test(ctx))
      continue;
    ...
    if (
      /\bbranch(es|ar|ado)?\s+(da|do|de|para|pra)\s+(empresa|escrit[oó]rio|filial|loja|unidade)\b/i.test(
        ctx,
      )
    )
      continue;
