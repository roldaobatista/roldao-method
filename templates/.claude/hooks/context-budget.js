...
(async () => {
  ...
  if (fs.existsSync(agents)) {
    ...
    if (n > AGENTS_LIMIT) {
      process.stderr.write(
        `[context-budget] AGENTS.md tem ${n} linhas (limite: ${AGENTS_LIMIT}). Consolide ou extraia conteúdo pra ADR/doc específico.\n`,
      );
    }
  }
  if (fs.existsSync(claude)) {
    ...
    if (n > CLAUDE_LIMIT) {
      process.stderr.write(
        `[context-budget] CLAUDE.md tem ${n} linhas (limite: ${CLAUDE_LIMIT}). Mova conteúdo de produto pra AGENTS.md.\n`,
      );
    }
  }
  process.exit(0);
