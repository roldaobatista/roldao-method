...
const SUGESTOES = [
  {
    ...
    descricao:
      'NF-e, NFC-e, NFS-e nacional (ABRASF), CT-e, MDF-e, Reforma Tributaria (LC 214/2025), split payment, obrigacao acessoria mensal',
    ...
    keywords:
      /\b(nfe?|nfse?|nfce?|cte?|mdfe?|sped(?:_fiscal|_contribuicoes)?|ecf|ecd|sefaz|abrasf|reforma\s+tributari|cbs|ibs|imposto\s+seletivo|split\s+payment|transportadora|frota|carga|operador\s+logistico)\b/i,
  },
  {
    addon: 'fintech-br',
    descricao: 'Pix completo (BR Code, webhook HMAC, idempotencia TxId), Open Finance',
    keywords:
      /\b(pix|endtoendid|e2eid|txid|psp|open\s+finance|bacen|sicoob|itau\s+pix|cobranca\s+pix|dict)\b/i,
  },
  {
    addon: 'lgpd-compliance',
    descricao:
      'DPO, RIPD, canal do titular, plano de incidente 72h, decisao automatizada (Art. 20)',
    keywords:
      /\b(lgpd|anpd|titular|dpo|ripd|base\s+legal|incidente\s+dados|decisao\s+automatizada|art\.?\s*20|transferencia\s+internacional)\b/i,
  },
  {
...
function buscarKeyword(projdir, re, max = 5) {
  try {
    const out = execFileSync(
      ...
      { stdio: ['ignore', 'pipe', 'ignore'], timeout: 5000, env: gitSafeEnv() },
    )
      .toString()
      .trim();
    if (!out) return [];
...
(async () => {
  ...
  for (const s of sugeridos) {
    ...
    process.stderr.write(
      `     encontrado em: ${s.exemplos.slice(0, 2).join(', ')}${s.exemplos.length > 2 ? '...' : ''}\n`,
    );
