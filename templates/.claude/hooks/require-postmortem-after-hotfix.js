...
(async () => {
  ...
  if (fs.existsSync(incidentesDir)) {
    try {
      for (const f of fs.readdirSync(incidentesDir)) {
        ...
        if (stat.mtimeMs > markerMaisAntigo) {
          postmortemRecente = true;
          break;
        }
      }
    } catch {
  ...
  process.stderr.write(
    `[require-postmortem-after-hotfix] BLOQUEADO: postmortem pendente apos /hotfix.\n\n`,
  );
  ...
  process.stderr.write(
    `Motivo: LGPD-006 da ate 72h pra notificar ANPD; cobertura do caso que escapou\n`,
  );
  ...
  process.stderr.write(
    `  3. Apos o INC ser mais novo que o marker, ciclo fecha automaticamente.\n\n`,
  );
  process.stderr.write(
    `Bypass de emergencia (use so se o postmortem JA foi feito fora do framework):\n`,
  );
  process.stderr.write(
    `  rm .claude/.runtime/needs-postmortem-* (apos block-destructive permitir)\n`,
  );
