...
const PII_RE =
  /\bcpf\b|\bcnpj\b|\brg\b|\bemail\b|\be[-_]mail\b|\btelefone\b|\bcelular\b|\bphone\b|\bendereco\b|\bnascimento\b|\bbirth(date|day)\b|\bnome_(completo|civil)\b|\bgenero\b|\betnia\b|\bbiometria\b|\bsaude\b|\bdiagnostico\b/i;
...
(async () => {
  ...
  if (fs.existsSync(decisionsDir)) {
    const namedAdr = findFirstMatch(decisionsDir, (n) =>
      /^ADR-.*(lgpd|base-legal).*\.md$/i.test(n),
    );
  ...
  if (!documentado) {
    ...
    if (fs.existsSync(featMark)) {
      try {
        ...
        if (usId) {
          const storyFile = findFirstMatch(
  ...
  process.stderr.write(
    `Arquivo ${filePath} menciona "${piiMatch[0]}" — pode ser dado pessoal (LGPD-001).\n`,
  );
  process.stderr.write(
