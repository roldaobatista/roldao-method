...
const EXCLUDED_PATH_RE =
  /\.md$|\/docs\/|README|CHANGELOG|\/test\/|\/tests\/|\.test\.|\.spec\.|\/fixtures\/|\/mocks\/|\.claude\/\.runtime\//;
...
const PROVIDER_RE =
  /(s3\.amazonaws\.com|s3-[a-z0-9-]+\.amazonaws\.com|storage\.googleapis\.com|gs:\/\/|blob\.core\.windows\.net|azure\.com|firebaseio\.com|firebasestorage\.googleapis\.com|sendgrid\.|mailgun\.|twilio\.|stripe\.com|sentry\.io|datadoghq\.com|mixpanel\.|segment\.com)/i;

// Regioes BR (sa-east-1 AWS, southamerica-east1 GCP, brazilsouth Azure)
const REGIAO_BR_RE =
  /(sa-east-1|sa-east|southamerica-east1|southamerica-west1|brazilsouth|brazil_south|brasil_sul)/i;

// Marker de tratamento documentado pra transferencia internacional
const DPA_RE =
  /LGPD-005:|transferencia[-_]internacional|DPA[-_]?assinado|standard[-_]contractual[-_]clauses|adequacy[-_]decision/i;
...
async function runHook(input) {
  ...
  process.stderr.write(
    `Arquivo ${filePath} usa provedor externo "${provMatch[0]}" e parece tocar dado pessoal,\n`,
  );
  process.stderr.write(
    `mas nao declarou regiao BR (sa-east-1 / southamerica-east1 / brazilsouth) nem DPA.\n\n`,
  );
  process.stderr.write(
    `LGPD-005 (transferencia internacional): dado pessoal de brasileiro saindo do Brasil exige\n`,
  );
  process.stderr.write(`base legal especifica + acordo (DPA) com o fornecedor.\n\n`);
  process.stderr.write(`Como destravar (escolha 1):\n`);
  process.stderr.write(
    `  (a) Usar regiao BR do provedor: AWS sa-east-1, GCP southamerica-east1, Azure brazilsouth\n`,
  );
  process.stderr.write(
    `  (b) Declarar DPA assinado: // LGPD-005: DPA-assinado com <fornecedor> em <data>\n`,
  );
  ...
  process.stderr.write(
    `Addon lgpd-compliance tem skill \`gerar-dpa-fornecedor\` que monta modelo.\n`,
  );
