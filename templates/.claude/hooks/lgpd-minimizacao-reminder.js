...
const PII_COLUMN_RE =
  /\b(cpf|cnpj|rg|email|e_mail|telefone|celular|phone|endereco|address|cep|nascimento|data_nascimento|birth_date|nome_completo|nome_civil|nome_mae|genero|etnia)\b/i;
const COLUMN_DECL_RE =
  /(CREATE\s+TABLE|ADD\s+COLUMN|alterTable|@column|@Column|@Field|column:|addColumn|t\.string|t\.text|String\s+@db|column\()/i;
...
async function runHook(input) {
  ...
  process.stderr.write(
    `Arquivo ${filePath} parece adicionar coluna de dado pessoal ("${piiMatch[0]}") sem justificativa.\n`,
  );
  ...
  process.stderr.write(
    `  2. Da pra usar versao mais grosseira? (faixa etaria > data nascimento; cidade > CEP)\n`,
  );
  ...
  process.stderr.write(
    `  (a) Comentario na migration: // LGPD-003: necessario para <feature/contrato>\n`,
  );
  process.stderr.write(
    `  (b) Frontmatter da story: base-legal: <consentimento|contrato|obrigacao legal>\n`,
  );
