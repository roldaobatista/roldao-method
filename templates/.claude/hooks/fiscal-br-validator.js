...
const EXCLUDED_PATH_RE =
  /\.md$|\/docs\/|README|CHANGELOG|\/test\/|\/tests\/|\/__tests__\/|\/spec\/|\/specs\/|\/e2e\/|\.test\.|\.spec\.|\.e2e\.|\/fixtures\/|\/mocks\/|\/__mocks__\//;
...
const FISCAL_001_REGEN =
  /(regenerate|regerar|rebuild|recreate|overwrite).{0,30}(nfe|nf-e|xml).{0,30}(autorizad|authorized|emitida)/i;
const FISCAL_001_ALTER =
  /(nfe|nf-e|xml).{0,30}(autorizad|authorized|emitida).{0,30}(update|alter|modify|edit|rewrite)/i;

// FISCAL-002: certificado/senha hardcoded
const FISCAL_002_CERT_PATH =
  /(certificate|certificado|pfx|p12|cert_path).{0,30}=.{0,30}["'][^"']+\.(pfx|p12|pem)["']/;
const FISCAL_002_CERT_PASS =
  /(cert_pass|cert_password|certificado_senha|pfx_pass|p12_pass).{0,5}=.{0,5}["'][^"']{3,}["']/i;
...
const FISCAL_003_STRING =
  /(tpAmb|tp_amb|ambiente|environment)\s*[=:]\s*["']?(producao|production|prod)["']?/i;
...
const F3_COMMENT_HOMOLOG_RE =
  /(\/\/|#|\/\*).{0,80}(homolog|sandbox|desenvolvimento|dev|teste|test|exemplo|example|comentario|documenta)/i;
const F3_CONST_REFERENCIAL_RE =
  /^\s*(const|let|var|final|static)\s+(tpAmb_PROD|TP_AMB_PROD|AMBIENTE_PRODUCAO|SEFAZ_PRODUCAO|PROD_ENV)/i;
...
const FISCAL_006_TAX_FIELDS =
  /\b(icms|iss|pis|cofins|cbs|ibs|imposto_seletivo|imposto_unico|aliquota|base_calculo|tributacao)\b/i;
const FISCAL_006_DECLARATION_RE =
  /FISCAL-006:\s*(transicao|pos-?2033|atual|novo-regime|split-payment|nao-aplica)/i;
// So dispara em arquivo que parece calculo (nome ou diretorio de tributacao/fiscal)
const FISCAL_006_RELEVANT_PATH =
  /(tributa|fiscal|imposto|calculo[-_]?(icms|iss|pis|cofins|cbs|ibs))/i;
...
const FISCAL_004_EMISSAO_RE =
  /\b(nfeAutorizacao|nfeAutorizacaoLote|nfceAutorizacao|cteAutorizacao|mdfeAutorizacao|enviarLote|transmitirNFe|sendNFe|emitirNFe|emitirNFCe|emitirCTe|emitirMDFe|enviar_lote_nfe|envia_lote)\b/i;
const FISCAL_004_DECLARATION_RE =
  /FISCAL-004:\s*(svc-an|svc-rs|epec|contingencia|fs-da|nao-aplica)/i;
...
(async () => {
  ...
  lines.forEach((line, i) => {
    ...
    if (FISCAL_001_REGEN.test(line))
      violations.push(`linha ${ln} [FISCAL-001]: regeracao de XML autorizado: ${line}`);
    if (FISCAL_001_ALTER.test(line))
      violations.push(`linha ${ln} [FISCAL-001]: alteracao de XML autorizado: ${line}`);
    ...
    if (
      FISCAL_003_NUMERIC.test(line) ||
      FISCAL_003_STRING.test(line) ||
      FISCAL_003_XML.test(line)
    ) {
      ...
      violations.push(
        `linha ${ln} [FISCAL-003]: ambiente de producao codificado direto (ou fallback ||1 perigoso): ${line}`,
      );
    }

    if (FISCAL_005_DIGIT.test(line)) {
      violations.push(
        `linha ${ln} [FISCAL-005]: regex CNPJ apenas numerica (pos jul/2026 aceita letras): ${line}`,
      );
    }
  ...
  if (violations.length > 0) {
    ...
    process.stderr.write(
      `              ou: // FISCAL-004: EPEC (Evento Previo de Emissao em Contingencia)\n`,
    );
    process.stderr.write(
      `              ou: // FISCAL-004: nao-aplica (modulo de leitura, nao emite)\n`,
    );
    process.stderr.write(
    ...
    process.stderr.write(
      `              // FISCAL-006: transicao (calcula ICMS atual E CBS/IBS novo em paralelo)\n`,
    );
    process.stderr.write(`              ou: // FISCAL-006: pos-2033 (so CBS/IBS)\n`);
    process.stderr.write(
      `              ou: // FISCAL-006: nao-aplica (codigo nao toca calculo, so persistencia)\n\n`,
    );
