# ROLDAO-METHOD no Gemini CLI

O Gemini CLI carrega este `GEMINI.md` automaticamente como contexto persistente do projeto. O contrato canônico continua em `AGENTS.md` (leia-o); este arquivo é o resumo operacional pro Gemini.
...
> Gemini CLI não tem hooks bloqueadores como o Claude Code — aqui a disciplina vale por instrução. O bloqueio mecânico (exit 2) só roda no Claude Code.

## Sequência obrigatória de agentes — VOCÊ é o gate (sem hook aqui)

No Claude Code, 5 hooks bloqueiam mecanicamente (exit 2) quem pula etapa. **No Gemini CLI não há bloqueio: VOCÊ é o gate.** Pular etapa = violação consciente do INV-AGENT-005 → entrega errada, bug em produção, dívida silenciosa.
