...
Depois, no seu assistente de IA: `/inicio` (projeto novo) ou `/brownfield` (já tem código rodando). `/help` lista os 35 roteiros disponíveis. Glossário sem jargão em [`docs/GLOSSARIO.md`](docs/GLOSSARIO.md). Quem não programa começa em [`docs/PARA-DONO-DE-PRODUTO.md`](docs/PARA-DONO-DE-PRODUTO.md). Nunca abriu o terminal? [`docs/ABRIR-TERMINAL.md`](docs/ABRIR-TERMINAL.md) explica.
