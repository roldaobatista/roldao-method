...
# Quickstart — pra desenvolvedor

5 minutos do zero ao primeiro `/feature`. Versão técnica.

> **Não é dev?** Vá pra [`docs/QUICKSTART-DONO.md`](QUICKSTART-DONO.md).
...
- `.claude/agents/` — 17 especialistas (analista, PM, UX, tech-lead, investigador, dev-senior, revisor, 3 auditores, fiscal-BR, tech-writer, DBA/dados, devops-infra, **qa-automation (Bia)**, **sre-on-call (Marcos)**).
- `.claude/hooks/` — 29 hooks bloqueadores + 4 soft warnings + 7 lifecycle + 1 utilitário (`_lib.js`) = **40 arquivos** em Node puro.
- `.claude/commands/` — 28 workflows.
- `.claude/skills/` — 19 skills BR core (CPF/CNPJ alfanum, chave NF-e, código IBGE de município, Pix, CEP, IE, boleto, BR Code, LGPD, ADR, traduzir jargão, brainstorming, fixture BR, mascarar dado pessoal, validar CNH/RENAVAM/título/conta-bancária/PIS-PASEP). Addons trazem +17 = 36 totais.
...
Confere que os 40 hooks estão instalados e com permissão de execução. Funciona em qualquer máquina com Node 18+ instalado (não precisa Git Bash, não precisa Python). Se acusar problema, abra issue.

**Opcional (perfil dev):** se quiser rodar a suíte completa de cenários adversariais:
...
```
ROLDAO v1.1.0 | Sonnet 4.6 | main | US-042 | dev-senior
```
...
`/help` lista os 28 comandos com códigos curtos e cenários comuns. Digite `/help FT` pra ver detalhes do `/feature`, `/help BG` pra `/bug`.
...
O core entrega 17 agentes + 19 skills BR. Pra domínios profundos, instale addons:
...
Addons disponíveis hoje: `fintech-br` (Pix + Open Finance), `fiscal-br-completo` (NF-e, NFC-e, SAT), `electron-br` (SQLite seguro, balança/impressora), `lgpd-compliance`, `esocial-completo`, `varejo-pdv-br`, `healthtech-br` (beta).
